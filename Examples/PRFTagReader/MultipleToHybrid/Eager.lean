...
lemma probEvent_multipleBadPathA_badReader_aux [Fintype Nonce] [Fintype Digest]
    ...
    (sB : UnlinkBadState TagId Nonce Digest)
    (_hAB : MultipleBadPathACoupling (TagId := TagId) (Nonce := Nonce) (Digest := Digest) c sB)
  ...
  | query_bind i k _ih =>
    ...
    -- Tag-query case (`i = Sum.inl tag`): `multipleBadQueryImplPathA_tag_run` unfolds to a step
    -- using `multipleBadAdvance`, which copies `sB.badReader` verbatim. `MultipleBadPathACoupling`
    -- is preserved by `MultipleBadPathACoupling_tag_advance_{some,none}`, and the
    -- `OracleComp.isQueryBoundP_query_bind_iff` decomposition gives the continuation budget `qR`
    -- (tag queries don't consume `·.isRight`).
    -- Reader-query case (`i = Sum.inr transcript`): `qR = qR' + 1` via the same decomposition.
    -- `multipleBadQueryImplPathA_reader_run` shows the step's bad-state output is the
    -- DETERMINISTIC value `multipleBadReaderAdvance transcript c sB`. The flip predicate
    -- `decide (∃ tag, c (tag, n) = some auth ∧ responses (tag, n) = none)` reduces under the
    -- coupling invariant `(a)`: any stale cell must have `readerTouched n = true`, hence was
    -- written by a *prior* reader query — its value is uniform-random over `Digest`, so the
    -- existence-over-`|TagId|` event has mass at most `|TagId|/|Digest|` under the cache
    -- distribution generated by the prefix of the computation. Eagerizing via
    -- `evalDist_simulateQ_multipleBadQueryImplPathA_run_eq_tableExtending` makes the
    -- "fresh-from-reader cells are uniform" claim formal: cells at `(tag, n)` with no session
    -- are slices of the uniform `g`. IH at `qR'` plus the per-step `|TagId|/|Digest|` slack
    -- closes the bind.
    sorry
