  ...
  | query_bind t f ih =>
    ...
    | inl tag =>
      ...
      · -- **Slot-available tag step (Path-A port).** Verbatim port of the original aux's
        ...
        -- on shared digests, with the bad branch absorbed into the widened
        -- `Pr[·.2.bad ∨ ·.2.badReader]` (via `multipleBadTableHandlerPathA_run_preserves_bad`
        -- + `Or.inl`) and the fresh branch closed by `MultipleHybridCoupling_tag_step` +
        ...
        have hLHS_eq :
            (do let gM ← $ᵗ (TagId × Nonce → Digest)
                let a ← multipleBadTableHandlerPathA (TagId := TagId) (Nonce := Nonce) (Digest := Digest)
                  (sessionsPerTag := sessionsPerTag) (OracleComp.tableExtending sM.2 gM)
                  (Sum.inl tag) (sM.1, sB)
                (fun z : Bool × (UnlinkState TagId × UnlinkBadState TagId Nonce Digest) => z.1) <$>
                  (simulateQ (multipleBadTableHandlerPathA (TagId := TagId) (Nonce := Nonce)
        ...
        · -- **Collision sub-case.** A past session of `tag` already drew `n`, so by
          -- `hInv.hbadcol` the bad-world `responses` cell `(tag, n)` is already filled. Hence
          -- `multipleBadAdvance` flips `bad := false || true = true`, then
          -- `multipleBadTableHandlerPathA_run_preserves_bad` propagates `bad = true` through the
          -- continuation, giving `Pr[r | ob n] = 1`. The off-`D` inequality then follows by
          -- bounding `Pr[q | my n] ≤ 1 = Pr[r | ob n] ≤ Pr[q | oc n] + Pr[r | ob n] + ε₂`.
            have hcell : (sB.responses (tag, n)).isSome = true := by
              rw [hInv.2.2.2.2.1]; exact hcoll
            ...
            have hPbadOne :
              ...
              obtain ⟨w, hw, hzeq⟩ := hzM
              subst hzeq
              exact Or.inl (multipleBadTableHandlerPathA_run_preserves_bad
                (OracleComp.tableExtending sM.2 gM)
                (f (some (⟨n, OracleComp.tableExtending sM.2 gM (tag, n)⟩ :
                    TagTranscript Nonce Digest)))
                (advM, multipleBadAdvance tag sB
            ...
            calc Pr[(· = true) | _]
                ≤ 1 := probEvent_le_one
              _ = Pr[(fun z : Bool × UnlinkBadState TagId Nonce Digest => z.2.bad ∨ z.2.badReader) | _] :=
                  hPbadOne.symm
              _ ≤ Pr[(· = true) | _] +
                    Pr[(fun z : Bool × UnlinkBadState TagId Nonce Digest =>
                        z.2.bad ∨ z.2.badReader) | _] + _ :=
                  le_add_right (le_add_left le_rfl)
        · -- **Off-collision sub-case.** No session of `tag` drew `n`. Combined with `hnD`, the
          -- multi cache must be `none` at `(tag, n)` (since `D = isSome ∧ ¬ collision`, and we
          -- are off `D` and off collision, so we are off `isSome`). Couple `gM(tag, n)` and
          ...
          · -- **Sub-case A (principal): the multi cache is unfilled at `(tag, n)`.** Couple the
            ...
            have hCacheBoundNew : ∀ u : Digest, ∀ tag' : TagId,
              ...
              · -- Off the freshly added cell, the multi cache is unchanged at `(tag', n')` and the
                ...
                · rintro ⟨sid, hsn⟩
                  ...
                  · rw [Function.update_of_ne hts]
                    exact hsn
            -- **Per-`u` Path-A coupling.** `MultipleBadPathACoupling_tag_advance_some` lifts
            -- the pre-step coupling `hAB : MultipleBadPathACoupling sM.2 sB` through the
            -- off-collision tag step at transcript `⟨n, u⟩`: the new cell `(tag, n)` is
            -- added to both the cache and `responses`, so the `(a)` premise fails for it;
            -- `readerTouched` is preserved unchanged, so `Q` carries over at every
            -- previously-touched nonce.
            have hABf : ∀ u : Digest,
                MultipleBadPathACoupling (TagId := TagId) (Nonce := Nonce) (Digest := Digest)
                  (sM.2.cacheQuery (tag, n) u)
                  (multipleBadAdvance tag sB
                    (some (⟨n, u⟩ : TagTranscript Nonce Digest))) := fun u =>
              MultipleBadPathACoupling_tag_advance_some sM.2 sB hAB tag
            ...
            have hIh : ∀ u : Digest,
              ...
              ih (some (⟨n, u⟩ : TagTranscript Nonce Digest)) qR qT' qRInit
                ...
                (advH n, sH.2.cacheQuery ((tag, sidH), n) u)
                (multipleBadAdvance tag sB (some (⟨n, u⟩ : TagTranscript Nonce Digest)))
                (hInvNew u) (hABf _) (hqRf _) (hqTf _) (hFreshNew u)
                (hCacheBoundNew u) hqRle
            -- **Cell-read simplification at the off-collision multi cell.** With `hMcellNone`,
            -- `tableExtending sM.2 gM (tag, n) = gM (tag, n)`: the multi-side cell read collapses
            -- to a direct lookup in the freshly drawn table `gM`.
...
The `MultipleBadPathACoupling` precondition is essential: without it, a pre-corrupted cache `c`
holding `some auth_0` at a non-session cell could deterministically flip `badReader` on the very
first reader query, making the bound `|TagId|/|Digest|` per query unattainable.
...
lemma probEvent_multipleBadPathA_badReader_aux [Fintype Nonce] [Fintype Digest]
  ...
  | pure b =>
    ...
    · simp [hbR]
  | query_bind i k ih =>
    -- Decompose `IsQueryBoundP` on the head bind.
    rw [OracleComp.isQueryBoundP_query_bind_iff] at hqR
    obtain ⟨hqR0, hqRk⟩ := hqR
    rw [multipleBadPathA_run_query_bind']
    cases i with
    | inl tag =>
      ...
      rw [multipleBadQueryImplPathA_tag_run tag, bind_assoc, probEvent_bind_eq_tsum]
      -- Pointwise IH: every `r ∈ support` of the inner step preserves the coupling and
      -- `badReader = sB.badReader` (via `multipleBadAdvance` copying `sB.badReader` verbatim).
      set K : ℝ≥0∞ := (if sB.badReader then 1 else 0) +
        ((qR * Fintype.card TagId : ℕ) : ℝ≥0∞) / (Fintype.card Digest : ℝ≥0∞) with hK
      have hPointwise : ∀ r,
          Pr[= r | (multipleIdealQueryImpl (TagId := TagId) (Nonce := Nonce) (Digest := Digest)
              (sessionsPerTag := sessionsPerTag) (Sum.inl tag)) ((s, c), sB).1] *
            Pr[fun z : Bool × MultipleBadState TagId Nonce Digest sessionsPerTag =>
                z.2.2.badReader |
              (pure (r.1, (r.2.1, r.2.2),
                multipleBadAdvance tag ((s, c), sB).2 r.1) >>= fun p =>
                (simulateQ (multipleBadQueryImplPathA (TagId := TagId) (Nonce := Nonce)
                  (Digest := Digest) (sessionsPerTag := sessionsPerTag)) (k p.1)).run p.2 :
                ProbComp (Bool × MultipleBadState TagId Nonce Digest sessionsPerTag))] ≤
            Pr[= r | (multipleIdealQueryImpl (TagId := TagId) (Nonce := Nonce) (Digest := Digest)
              (sessionsPerTag := sessionsPerTag) (Sum.inl tag)) ((s, c), sB).1] * K := by
        intro r
        -- Filter out non-support `r`: there `Pr[= r | step] = 0`, so the inequality is trivial.
        by_cases hr_mem : r ∈ support (multipleIdealQueryImpl (TagId := TagId) (Nonce := Nonce)
            (Digest := Digest) (sessionsPerTag := sessionsPerTag) (Sum.inl tag) ((s, c), sB).1)
        · refine mul_le_mul_left' ?_ _
          rw [pure_bind]
          -- Derive the coupling at the post-step cache `r.2.2` and `multipleBadAdvance tag sB r.1`.
          -- Case-split on whether the tag still has session budget.
          have hCoupling : MultipleBadPathACoupling r.2.2
              (multipleBadAdvance tag ((s, c), sB).2 r.1) := by
            by_cases hslot : s.sessionsUsed tag < sessionsPerTag
            · -- Slot available: support reduces to `(some ⟨n, d⟩, advU, c')` for some
              -- `(d, c') ∈ support (idealCacheStep c (tag, n))`.
              rw [multipleIdealQueryImpl_tag_run_of_lt tag s c hslot] at hr_mem
              obtain ⟨n, _, hr_mem'⟩ := (mem_support_bind_iff _ _ _).mp hr_mem
              obtain ⟨⟨d, c'⟩, hstep, hr_eq⟩ := (mem_support_bind_iff _ _ _).mp hr_mem'
              -- `r = (some ⟨n, d⟩, advU, c')` with `(d, c') ∈ support (idealCacheStep ...)`.
              have hr_eq' : r = _ := (mem_support_pure_iff _ _).mp hr_eq
              subst hr_eq'
              -- Reuse `MultipleBadPathACoupling_tag_advance_some` structure inline:
              -- the post-step cache adds `(tag, n) ↦ d` (or keeps the existing entry).
              obtain ⟨hQ, ha⟩ := hAB
              refine ⟨?_, ?_⟩
              · intro n' hRT tag'
                dsimp only
                ...
                by_cases hkey : (tag', n') = (tag, n)
                · rw [hkey]
                  exact idealCacheStep_cache_self_dom c (tag, n) (d, c') hstep
                · change ((d, c').2 (tag', n')).isSome = true
                  rw [idealCacheStep_cache_off c (tag, n) (d, c') hstep (tag', n') hkey]
                  exact hcprev
              · intro tag' n' h1 h2
                dsimp only at h1 h2
                by_cases hkey : (tag', n') = (tag, n)
                · exfalso
                  have hsome : (multipleBadAdvance tag sB
                      (some (⟨n, d⟩ : TagTranscript Nonce Digest))).responses (tag', n') =
                      some (d :: Option.getD (sB.responses (tag, n)) []) := by
                    rw [hkey]
                    change (sB.responses.cacheQuery (tag, n) _) (tag, n) = _
                    rw [QueryCache.cacheQuery_self]
                  rw [hsome] at h2; cases h2
                · have hc'_eq : c' (tag', n') = c (tag', n') :=
                    idealCacheStep_cache_off c (tag, n) (d, c') hstep (tag', n') hkey
                  have hresp_eq : (sB.responses.cacheQuery (tag, n)
                      (d :: Option.getD (sB.responses (tag, n)) [])) (tag', n') =
            ...
            · -- Slot exhausted: support is the singleton `(none, s, c)`.
              rw [multipleIdealQueryImpl_tag_run_of_not_lt tag s c hslot] at hr_mem
              have hr_eq : r = _ := (mem_support_pure_iff _ _).mp hr_mem
              -- `r = (none, s, c)`, so `multipleBadAdvance tag sB none = sB` and `r.2.2 = c`.
              subst hr_eq
              exact MultipleBadPathACoupling_tag_advance_none c sB hAB tag
          -- `badReader` is preserved by `multipleBadAdvance` (literally, by both branches).
          have hbR : (multipleBadAdvance tag ((s, c), sB).2 r.1).badReader = sB.badReader := by
            cases r.1 <;> rfl
          -- Apply IH and rewrite the indicator to `sB.badReader`.
          refine (ih r.1 qR r.2.1 r.2.2 _ hCoupling (hqRk' r.1)).trans ?_
          rw [hbR]
        · -- Out of support: the multiplier `Pr[= r | step]` is zero.
          rw [probOutput_eq_zero_of_not_mem_support hr_mem, zero_mul, zero_mul]
      refine le_trans (ENNReal.tsum_le_tsum hPointwise) ?_
...
lemma probEvent_multipleBadPathA_badReader_le [Fintype Nonce] [Fintype Digest]
  ...
  have h := probEvent_multipleBadPathA_badReader_aux (sessionsPerTag := sessionsPerTag)
    ...
    UnlinkBadState.init MultipleBadPathACoupling_init hqReader
