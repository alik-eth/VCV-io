...
private lemma multipleBadStep_bad_le
    (tag : TagId) (s : UnlinkState TagId)
    (c : ((TagId × Nonce) →ₒ Digest).QueryCache)
    (sB : UnlinkBadState TagId Nonce Digest)
    (maxNonceProb : ℝ≥0∞)
...
private lemma multipleBadStep_sessionsUsed_eq
    (tag : TagId) (s : UnlinkState TagId)
    (c : ((TagId × Nonce) →ₒ Digest).QueryCache)
    (sB : UnlinkBadState TagId Nonce Digest)
...
private lemma multipleBadStep_reader_state_eq
    (transcript : TagTranscript Nonce Digest)
    (s : UnlinkState TagId)
    (c : ((TagId × Nonce) →ₒ Digest).QueryCache)
    (sB : UnlinkBadState TagId Nonce Digest) :
...
private lemma multipleBadStep_preserves
    (t : (UnlinkOracleSpec TagId Nonce Digest).Domain)
    (s : UnlinkState TagId)
    (c : ((TagId × Nonce) →ₒ Digest).QueryCache)
    (sB : UnlinkBadState TagId Nonce Digest)
...
private lemma simulateQ_multipleBad_prob_le
    (adversary : UnlinkAdversary TagId Nonce Digest)
    (maxNonceProb : ℝ≥0∞)
    (hmax : ∀ n : Nonce, Pr[= n | ($ᵗ Nonce : ProbComp Nonce)] ≤ maxNonceProb)
    (s : UnlinkState TagId)
...
private theorem multipleBad_bad_le_sessionCollisionBound
    (adversary : UnlinkAdversary TagId Nonce Digest)
    (maxNonceProb : ℝ)
...
theorem unlinkabilityAdvantage_le_two_prf_plus_uniform_sessionCollisionBound
  ...
  obtain ⟨multiAdv, singleAdv, h⟩ :=
    unlinkabilityAdvantage_le_two_prf_plus_sessionCollisionBound prfs adversary qReader qTag
      hqReader hqTag hdist ((Fintype.card Nonce : ℝ)⁻¹) hmax
