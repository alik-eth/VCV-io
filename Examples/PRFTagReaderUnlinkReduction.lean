...
`multipleBadQueryImpl` collision term, and the chained reader/tag slack terms. The bound holds for
every adversary.
...
plus the five unconditional reader/tag cell and nonce-aliasing slack terms. The bound holds for
every adversary. -/
